...
const renderDosenCard = (dosen: ProcessedDosen) => (
  ...
  >
    <div className="w-48 aspect-[3/4] rounded-xl overflow-hidden bg-gray-100 mb-4 relative"> {/* Lebar foto diperbesar dari w-40 ke w-48 */}
      {dosen.foto ? (
        <img
          src={dosen.foto}
          alt={dosen.nama}
...
export default function D4TeknikEnergi() {
  ...
  const [dosenD4Energi, setDosenD4Energi] = useState<ProcessedDosen[]>([]);
  const [loadingDosen, setLoadingDosen] = useState(true);
  const [errorDosen, setErrorDosen] = useState<string | null>(null);

  // Effect to fetch Prodi Details from cms_prodi for "D4 Teknik Energi"
  useEffect(() => {
    const fetchProdiDetails = async () => {
      ...
      try {
        const { data, error } = await supabase
          .from('cms_prodi')
          .select('*')
          .eq('title', 'D4 Teknik Energi') // Fetch specifically for D4 Teknik Energi
          .single();

        if (error) {
          // console.error('Error fetching D4 Teknik Energi details:', JSON.stringify(error, null, 2));
          if (error.code === 'PGRST116') { // PostgREST error for "single() row not found"
            setErrorProdi(`Program studi "D4 Teknik Energi" tidak ditemukan di CMS.`);
          } else {
            setErrorProdi(`Gagal memuat detail Program Studi D4 Teknik Energi: ${error.message || 'Kesalahan tidak diketahui dari database.'}`);
          }
        } else if (data) {
          setProdiDetails(data);
        } else {
          setErrorProdi(`Program studi "D4 Teknik Energi" tidak ditemukan (data kosong).`);
        }
      } catch (err: any) {
        // console.error('Unexpected error fetching D4 Teknik Energi details:', err);
  ...
  useEffect(() => {
    const fetchDosenD4Energi = async () => {
      setLoadingDosen(true);
      setErrorDosen(null); // Clear previous errors
      try {
        // Fetch all dosen data - email and prodi are still selected here
        // because they are needed for filtering/categorization logic in the fetch.
        const { data, error } = await supabase
          .from('dosen')
          .select('id_dsn, nama, nip, nuptk, nidn, foto, role') // Added nuptk
          .eq('prodi', 'D4 Teknik Energi') // Filter by prodi on the server
        ...
        if (error) {
          ...
          setErrorDosen('Gagal memuat data dosen D4 Teknik Energi');
        ...
        positionedDosen.sort((a, b) => {
          ...
          const orderA = a.position_order ?? Infinity; // Dosen interface allows null, so keep ?? Infinity
        ...
        // Prepend the special lecturer if found
        if (specialLecturerData) {
          finalSortedDosen = [specialLecturerData, ...finalSortedDosen];
        }
        setDosenD4Energi(finalSortedDosen);

      } catch (err: any) {
        // console.error('Unexpected error fetching D4 Energi dosen:', err);
  ...
  return (
    <main className="bg-gradient-to-b from-gray-50 to-gray-200 min-h-screen py-8 px-2 md:px-4 font-sans"> {/* Padding horizontal dikurangi lagi */}

      {/* Informasi Program Studi */}
      <section className="px-2 md:px-4">
        <h1 className="text-3xl font-bold text-maroon-700 mb-8 text-center">
          Program Studi {prodiDetails?.title || "D4 Teknik Energi"}
        </h1>
      ...
      {prodiDetails?.file ? (
        <section className="flex justify-center mb-10">
          <img
            ...
            alt={`Sertifikat Akreditasi ${prodiDetails.title || "D4 Teknik Energi"}`}
      ...
      {/* Dosen D4 Teknik Energi Section */}
      <section className="max-w-7xl mx-auto space-y-8">
        <h1 className="text-3xl font-bold text-maroon-700 mb-8 text-center">
          Dosen D4 Teknik Energi
        </h1>
        ...
        {!loadingDosen && !errorDosen && dosenD4Energi.length === 0 && (
          <div className="text-center p-8 bg-gray-100 text-gray-700 rounded-lg shadow-md w-full">
            ...
            <p>Tidak ada data dosen untuk Program Studi D4 Teknik Energi.</p>
          </div>
        )}
        ...
        {!loadingDosen && !errorDosen && dosenD4Energi.length > 0 && (
          ...
          <div className="flex justify-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 w-fit"> {/* Mengurangi gap dari 8 menjadi 4 */}
              {dosenD4Energi.map(renderDosenCard)}
            </div>
          </div>
        )}
      </section>
      ...
      <section className="px-2 md:px-4 mt-12"> {/* Increased top margin */}
        ...
        {[1, 2, 3, 4, 5, 6, 7, 8].map((semester) => ( // Looping for 8 semesters
          <div key={semester} className="mb-8"> {/* Container for each semester block */}
            ...
            <div className="flex"> {/* Flex container to center the image */}
              ...
              <img
                src={`/kurikulumD4tekenergiS${semester}.jpg`} // Assuming image names follow this pattern
