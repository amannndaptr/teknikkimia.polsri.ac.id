...
export default function S2TeknikEnergiTerbarukan() {
  ...
  const [dosenS2EBT, setDosenS2EBT] = useState<ProcessedDosen[]>([]);
  const [loadingDosen, setLoadingDosen] = useState(true);
  const [errorDosen, setErrorDosen] = useState<string | null>(null);

  // Effect to fetch Prodi Details from cms_prodi for "S2 Energi Terbarukan"
  useEffect(() => {
    const fetchProdiDetails = async () => {
      ...
      try {
        const { data, error } = await supabase
          .from('cms_prodi')
          .select('*')
          .eq('title', 'S2 Energi Terbarukan') // Fetch specifically for S2 Energi Terbarukan
          .single();

        if (error) {
          // console.error('Error fetching S2 Energi Terbarukan details:', JSON.stringify(error, null, 2));
          if (error.code === 'PGRST116') { // PostgREST error for "single() row not found"
            setErrorProdi(`Program studi "S2 Energi Terbarukan" tidak ditemukan di CMS.`);
          } else {
            setErrorProdi(`Gagal memuat detail Program Studi S2 Energi Terbarukan: ${error.message || 'Kesalahan tidak diketahui dari database.'}`);
          }
        } else if (data) {
          setProdiDetails(data);
        } else {
          setErrorProdi(`Program studi "S2 Energi Terbarukan" tidak ditemukan (data kosong).`);
        }
      } catch (err: any) {
        // console.error('Unexpected error fetching S2 Energi Terbarukan details:', err);
    ...
    const fetchDosenS2EBT = async () => {
      ...
      try {
        ...
        const { data, error } = await supabase
          ...
          .eq('prodi', 'S2 Energi Terbarukan')           // Filter by prodi on the server
        ...
        if (error) {
          ...
          setErrorDosen('Gagal memuat data dosen S2 Teknik Energi Terbarukan');
          setLoadingDosen(false);
          return;
        }
        // console.log('[S2 EBT Dosen] Raw data fetched from "dosen" table:', data); // Log 1: Raw data
        ...
        // The console.log for raw data can be removed or kept for debugging if needed.
        ...
        // console.log('[S2 EBT Dosen] Filtered and Sorted Dosen for S2 EBT:', sortedDosen); // Log 3: Final list
        if (specialLecturerData) {
          finalSortedDosen = [specialLecturerData, ...finalSortedDosen];
        }
        
        setDosenS2EBT(finalSortedDosen);

      } catch (err: any) {
        // console.error('Unexpected error fetching S2 EBT dosen:', err);
    ...
    <main className="bg-gradient-to-b from-gray-50 to-gray-200 min-h-screen py-8 px-2 md:px-4 font-sans"> {/* Padding disamakan */}
      ...
      <section className="px-2 md:px-4">
        <h1 className="text-3xl font-bold text-maroon-700 mb-8 text-center">
          Program Studi {prodiDetails?.title || "S2 Energi Terbarukan"}
        </h1>
      ...
      {prodiDetails?.file ? (
        <section className="flex justify-center mb-10">
          <img
            ...
            alt={`Sertifikat Akreditasi ${prodiDetails.title || "S2 Energi Terbarukan"}`}
      ...
      {/* Dosen S2 Teknik Energi Terbarukan Section */}
      <section className="max-w-7xl mx-auto space-y-8">
        <h1 className="text-3xl font-bold text-maroon-700 mb-8 text-center">
          Dosen S2 Energi Terbarukan
        </h1>
        ...
        {!loadingDosen && !errorDosen && dosenS2EBT.length === 0 && (
          <div className="text-center p-8 bg-gray-100 text-gray-700 rounded-lg shadow-md w-full">
            ...
            <p>Tidak ada data dosen untuk Program Studi S2 Teknik Energi Terbarukan.</p>
          </div>
        )}

        {!loadingDosen && !errorDosen && dosenS2EBT.length > 0 && (
          // Flex container to center the grid
          <div className="flex justify-center"> {/* justify-left diubah ke justify-center */}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 w-fit"> {/* Grid dan gap disamakan */}
              {dosenS2EBT.map(renderDosenCard)}
            </div>
          </div>
        )}
      </section>
      ...
      <section className="px-2 md:px-4 mt-12"> {/* Increased top margin */}
        ...
        <div className="flex"> {/* Centered the image */}
          {/* [Image of Kurikulum Semester] */}
          {/* Assuming there is only one image for the S2 curriculum */}
          <img
            src={`/kurikulumS2EnergiTerbarukan.jpg`}
            alt={`Kurikulum Semester`}
