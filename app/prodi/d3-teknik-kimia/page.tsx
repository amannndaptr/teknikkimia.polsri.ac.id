...
const renderDosenCard = (dosen: ProcessedDosen) => (
  ...
  >
    <div className="w-48 aspect-[3/4] rounded-xl overflow-hidden bg-gray-100 mb-4 relative"> {/* Lebar foto disamakan dengan D4 */}
      {dosen.foto ? (
        <img
          src={dosen.foto}
          alt={dosen.nama}
...
export default function D3TeknikKimia() {
  ...
  const [dosenD3, setDosenD3] = useState<ProcessedDosen[]>([]);
  const [loadingDosen, setLoadingDosen] = useState(true);
  const [errorDosen, setErrorDosen] = useState<string | null>(null);

  // Effect to fetch Prodi Details from cms_prodi for "D3 Teknik Kimia"
  useEffect(() => {
    const fetchProdiDetails = async () => {
      ...
      try {
        const { data, error } = await supabase
          .from('cms_prodi')
          .select('*')
          .eq('title', 'D3 Teknik Kimia') // Fetch specifically for D3 Teknik Kimia
          .single();

        if (error) {
          // console.error('Error fetching D3 Teknik Kimia details:', error);
          setErrorProdi(`Gagal memuat detail Program Studi D3 Teknik Kimia: ${error.message}`);
        } else {
          setProdiDetails(data);
        }
      } catch (err: any) {
        // console.error('Unexpected error fetching D3 Teknik Kimia details:', err);
    ...
    const fetchDosenD3 = async () => {
      ...
      try {
        ...
        const { data, error } = await supabase
          ...
          .eq('prodi', 'D3 Teknik Kimia') // Filter by prodi on the server
        ...
        if (error) {
          ...
          setErrorDosen('Gagal memuat data dosen D3 Teknik Kimia');
        ...
        setDosenD3(finalSortedDosen);

      } catch (err: any) {
        // console.error('Unexpected error fetching D3 dosen:', err);
  ...
  return (
    <main className="bg-gradient-to-b from-gray-50 to-gray-200 min-h-screen py-8 px-2 md:px-4 font-sans"> {/* Padding disamakan dengan D4 */}

      {/* Informasi Program Studi */}
      <section className="px-2 md:px-4">
        <h1 className="text-3xl font-bold text-maroon-700 mb-8 text-center">
          Program Studi {prodiDetails?.title || "D3 Teknik Kimia"}
        </h1>
      ...
      {prodiDetails?.file ? (
        <section className="flex justify-center mb-10">
          <img
            ...
            alt={`Sertifikat Akreditasi ${prodiDetails.title || "D3 Teknik Kimia"}`}
      ...
      {/* Dosen D3 Teknik Kimia Section */}
      <section className="max-w-7xl mx-auto space-y-8">
        <h1 className="text-3xl font-bold text-maroon-700 mb-8 text-center">
          Dosen D3 Teknik Kimia
        </h1>
        ...
        {!loadingDosen && !errorDosen && dosenD3.length === 0 && (
          <div className="text-center p-8 bg-gray-100 text-gray-700 rounded-lg shadow-md w-full">
            ...
            <p>Tidak ada data dosen untuk Program Studi D3 Teknik Kimia.</p>
          </div>
        )}
        ...
        {!loadingDosen && !errorDosen && dosenD3.length > 0 && (
          ...
          <div className="flex justify-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 w-fit"> {/* Grid dan gap disamakan dengan D4 */}
              {dosenD3.map(renderDosenCard)}
            </div>
          </div>
        )}
      </section>
        ...
        {[1, 2, 3, 4, 5, 6].map((semester) => (
          <div key={semester} className="mb-8"> {/* Container for each semester block */}
            ...
            <div className="flex"> {/* Flex container to center the image */}
              ...
              <img
                src={`/kurikulumD3tekkimS${semester}.jpg`}
