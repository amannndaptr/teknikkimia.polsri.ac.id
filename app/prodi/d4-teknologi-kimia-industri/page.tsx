...
export default function D4TeknologiKimiaIndustri() {
  ...
  const [dosenD4TKI, setDosenD4TKI] = useState<ProcessedDosen[]>([]);
  const [loadingDosen, setLoadingDosen] = useState(true);
  const [errorDosen, setErrorDosen] = useState<string | null>(null);

  // Effect to fetch Prodi Details from cms_prodi for "D4 Teknologi Kimia Industri"
  useEffect(() => {
    const fetchProdiDetails = async () => {
      ...
      try {
        const { data, error } = await supabase
          .from('cms_prodi')
          .select('*')
          .eq('title', 'D4 Teknologi Kimia Industri') // Fetch specifically for D4 Teknologi Kimia Industri
          .single();

        if (error) {
          // console.error('Error fetching D4 Teknologi Kimia Industri details:', JSON.stringify(error, null, 2));
          if (error.code === 'PGRST116') { // PostgREST error for "single() row not found"
            setErrorProdi(`Program studi "D4 Teknologi Kimia Industri" tidak ditemukan di CMS.`);
          } else {
            setErrorProdi(`Gagal memuat detail Program Studi D4 Teknologi Kimia Industri: ${error.message || 'Kesalahan tidak diketahui dari database.'}`);
          }
        } else if (data) {
          setProdiDetails(data);
        } else {
          setErrorProdi(`Program studi "D4 Teknologi Kimia Industri" tidak ditemukan (data kosong).`);
        }
      } catch (err: any) {
        // console.error('Unexpected error fetching D4 Teknologi Kimia Industri details:', err);
    ...
    const fetchDosenD4TKI = async () => {
      ...
      try {
        ...
        const { data, error } = await supabase
          ...
          .eq('prodi', 'D4 Teknologi Kimia Industri')    // Filter by prodi on the server
        ...
        if (error) {
          ...
          setErrorDosen('Gagal memuat data dosen D4 Teknologi Kimia Industri');
        ...
        setDosenD4TKI(finalSortedDosen);

      } catch (err: any) {
        // console.error('Unexpected error fetching D4 TKI dosen:', err);
    ...
    <main className="bg-gradient-to-b from-gray-50 to-gray-200 min-h-screen py-8 px-2 md:px-4 font-sans"> {/* Padding disamakan */}
      ...
      <section className="px-2 md:px-4">
        <h1 className="text-3xl font-bold text-maroon-700 mb-8 text-center">
          Program Studi {prodiDetails?.title || "D4 Teknologi Kimia Industri"}
        </h1>
      ...
      {prodiDetails?.file ? (
        <section className="flex justify-center mb-10">
          <img
            ...
            alt={`Sertifikat Akreditasi ${prodiDetails.title || "D4 Teknologi Kimia Industri"}`}
      ...
      {/* Dosen D4 Teknologi Kimia Industri Section */}
      <section className="max-w-7xl mx-auto space-y-8">
        <h1 className="text-3xl font-bold text-maroon-700 mb-8 text-center">
          Dosen D4 Teknologi Kimia Industri
        </h1>
        ...
        {!loadingDosen && !errorDosen && dosenD4TKI.length === 0 && (
          <div className="text-center p-8 bg-gray-100 text-gray-700 rounded-lg shadow-md w-full">
            ...
            <p>Tidak ada data dosen untuk Program Studi D4 Teknologi Kimia Industri.</p>
          </div>
        )}
        ...
        {!loadingDosen && !errorDosen && dosenD4TKI.length > 0 && (
          ...
          <div className="flex justify-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 w-fit"> {/* Grid dan gap disamakan */}
              {dosenD4TKI.map(renderDosenCard)}
            </div>
          </div>
        )}
      </section>
        ...
        {[1, 2, 3, 4, 5, 6, 7, 8].map((semester) => ( // Looping for 8 semesters
          <div key={semester} className="mb-8"> {/* Container for each semester block */}
            ...
            <div className="flex"> {/* Flex container to center the image */}
              ...
              <img
                src={`/kurikulumD4tekkimindusS${semester}.jpg`} // Assuming image names follow this pattern
